...
class VLCTimeTest: XCTestCase {
    ...
    func testCompare() {
        ...
        XCTAssert(greater.compare(smaller) == ComparisonResult.orderedDescending)
        XCTAssert(smaller.compare(greater) == ComparisonResult.orderedAscending)
        XCTAssert(greater.compare(greater) == ComparisonResult.orderedSame)
        XCTAssertEqual(greater.isEqual(smaller), false)
    }
    
    func testHash() {
        let time = VLCTime(int: 10)
        XCTAssert(time?.hash() != nil)
    }
