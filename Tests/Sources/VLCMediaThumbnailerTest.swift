...
class VLCMediaThumbnailerTest: XCTestCase {
    ...
    func testInit() throws {
        let tests = [Video.test1, Video.invalid]
    ...
    func testInitWithLibrary() throws {
        let tests = [VLCLibrary(options: nil), VLCLibrary.shared()]
        
        for library in tests {
            let delegate = MockThumbnailerDelegate()
            ...
            XCTAssertNotNil(thumbnailer.delegate)
            XCTAssertEqual(thumbnailer.media, media)
            XCTAssertEqual(thumbnailer.libVLCinstance, library?.instance)
        }
    }
    ...
    func testDelegateDidFinishThumbnail() throws {
        let tests: [(video: Video, height: CGFloat, width: CGFloat)] = [
            (Video.test1, 417, 240),
            (Video.test2, 427, 240),
            (Video.test3, 427, 240),
            (Video.test4, 427, 240)
